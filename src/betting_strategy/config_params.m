function [ A, b, Aeq, beq, lb, ub, intcon, f, kellies, bookmaker_row_vector, prob_row_vector] = config_params( total_budget, single_bet_budget, p_games, bookmakers)
...
A = zeros(1 + 2 * num_of_games * num_of_bookmakers * num_of_results,...
    num_of_games * num_of_bookmakers * num_of_results);
b = zeros(1 + 2 * num_of_games * num_of_bookmakers * num_of_results, 1);
...
for i = 1 : num_of_games
    for j = 1 : num_of_bookmakers
        for k = 1 : num_of_results
            ...
            b(n) = single_bet_budget;  % x_i * kelly * total_budget <= single_game_budget
            n = n + 1;
            
            A(n, (i - 1) * num_of_bookmakers *num_of_results + (j - 1) * num_of_results + k) = -kelly * total_budget;
            b(n) = 0;    % x_i * kelly * total_budget >= 0
            n = n + 1;
        end
    end
end
    ...
    for j = 1 : num_of_bookmakers
        for k = 1 : num_of_results
            A(end, (i - 1) * num_of_bookmakers *num_of_results + (j - 1) * num_of_results + k) ...
                = kellies((i - 1) * num_of_bookmakers *num_of_results + (j - 1) * num_of_results + k) * total_budget;
    ...
    for j = 1 : num_of_bookmakers  
        for k = 1 : num_of_results 
            f(1, (i - 1) * num_of_bookmakers *num_of_results + (j - 1) * num_of_results + k) =...
                ...
                + (j - 1) * num_of_results + k) * total_budget * (bookmakers(i, k, j) - 1));  
        end
    end
end
